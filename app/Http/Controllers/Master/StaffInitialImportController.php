...
class StaffInitialImportController extends Controller
{
    ...
    private function templateSpreadsheet(): Spreadsheet
    {
        ...
        $sheet->fromArray([
            ['PT001', '山田 花子', 'アルバイト', '2026-04-01', null, '46', '2026-04-01', null, 1300, '2026-04-01', null, 500, '非課税', '2026-04-01', null, '甲欄', 1, '2026-04-01', null],
            ['PT001', '山田 花子', 'アルバイト', '2026-04-01', null, 'オニカイ', '2026-04-01', null, 1300, '2026-04-01', null, 600, '課税', '2026-04-01', null, '甲欄', 1, '2026-04-01', null],
            ['EMP001', '佐藤 太郎', '社員', '2026-04-01', null, '46', '2026-04-01', null, null, null, null, null, null, null, null, null, null, null, null],
        ], null, 'A2');
        ...
        foreach (range('A', $lastColumn) as $column) {
            $sheet->getColumnDimension($column)->setWidth(in_array($column, ['B', 'F'], true) ? 18 : 14);
        }
        ...
        $guide->fromArray([
            ...
            ['2. スタッフキーが同じ行は、同一スタッフとしてまとめて登録されます。'],
            ['3. 複数店舗へ所属する場合は、同じスタッフキーで行を追加してください。'],
            ['4. 店舗名は、システムへ登録済みの名称と完全一致させてください。'],
            ['5. 日付はYYYY-MM-DD、金額と扶養人数は0以上の整数で入力してください。'],
            ['6. 雇用区分は「社員」「アルバイト」、課税区分は「課税」「非課税」、所得税区分は「甲欄」「乙欄」です。'],
            ['7. 1件でもエラーがある場合、全件を登録せず行番号付きでエラーを表示します。'],
            ['8. このインポートは新規登録専用です。同じファイルを再送するとスタッフが重複するため注意してください。'],
        ], null, 'A1');
        $guide->getStyle('A1')->getFont()->setBold(true)->setSize(16)->getColor()->setRGB('009AD8');
        $guide->getColumnDimension('A')->setWidth(110);
        $guide->getStyle('A1:A9')->getAlignment()->setWrapText(true)->setVertical(Alignment::VERTICAL_TOP);
